		...
		try {
			if (selection == 1) {
				...
				if (time >= 23 && time <= 1) {
					...
					throw new PassengerDetailException("BookingNotAllowedException ");
				} else {
				...
				if (inputage > 5) {
					...
					if (number > 0 && number <= 6) {
						...
						throw new PassengerDetailException("normalTicketBookingException");
					}
				}
				else {
				...
				if (d.getHours() >= 10 && d.getHours() <= 12) {
					...
					if (inputage > 5) {
						...
						if (number > 0 && number <= 4) {
							...
							throw new PassengerDetailException("booking limit exceed");
						}
					} else {
						System.out.println("age exception");
					}
					...
					throw new PassengerDetailException("TatkalTicketBookingException");
				}
			} 
		} 
		catch (Exception e) {
